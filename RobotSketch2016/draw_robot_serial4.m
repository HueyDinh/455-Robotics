function draw_robot_serial4(t,d,f,a,j_type,frame_type,j_centered,edges,R_rad,T0,varargin)
...
%   R_rad ------ R joint radius (if <= 0, it is calculated)
%
%   varargin:
...
if length(varargin) == 2               %x and y polygon coordinates inputted
    Hx = varargin{1};                  %x coordinate
    Hy = varargin{2};                  %y coordinate
    Hn = length(Hx);                   %number of vertices
end %if
...
if  R_rad <= 0                 %calc R joint radius > 0 if not yet set
...
s = (d>=0)-(d<0);                           %nonzero sign of d (+1/-1),for properly extending the offsets when negative
...
for i=1:n+1                                         %i counts the RPG g-joints and frame number beyond base
    ...
    %draw a(i-1) common normal or platform
    if ((a(I-1)>=0)|((j_type(I-1)=='H')&(a(I-1)>=0)));                                                  %draw if a(i-1) >= 0 and not a platform link
        draw_part('a', a_rad, sides, a_min(I-1), a_max(I-1),( edges=='E'), Ti_1, a_color) %draw common normal
        if j_type(I-1)=='H'                                                                 %draw platform link
            draw_part('H', R_rad, sides, -a_rad/2, a_rad/2,( edges=='E'), Ti_1, a_color,Hx,Hy)%draw platform link            
        end %if
    end %if
    ...
    if     (j_type(I) == 'R')                                                       %R joint (***kludge***)
        draw_part('R', R_rad, j_sides(I), j_min(I), j_max(I), edge(I), Ti, R_color) %draw R joint
    elseif j_type(I-1) == 'H'
        draw_part('R', R_rad, j_sides(I), j_min(I), j_max(I), edge(I), Ti_1, R_color) %draw R joint        
